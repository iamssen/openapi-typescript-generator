...
export interface GetAllPetsRequestParameters {
  lastUpdated?: Date;
}
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const getAllPetsRequest = ({
  basePath = '',
  querystringStringify = querystring,
  username,
  password,
  ...
  requestParameters: GetAllPetsRequestParameters,
): FetchParameters => {
  const queryParameters: UrlQueryInput = {};
  ...
  if (requestParameters.lastUpdated !== undefined) {
    queryParameters[
      'lastUpdated'
    ] = (requestParameters.lastUpdated as any).toISOString();
  }
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/pet/getAll`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const getAllPetsResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<Array<Pet>> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObjectList('Pet'));
...
export async function getAllPets(
  ...
  return pipe(
    getAllPetsRequest(configuration),
    fetchRequest(configuration),
    getAllPetsResponse(configuration),
  )(requestParameters);
}
