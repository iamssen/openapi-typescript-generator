...
import type { SomeObject } from '../models';
import { typeSerializer } from '../runtime';

/* eslint-disable */

export interface TestMeOpRequestParameters {
  body?: SomeObject;
}
...
export const testMeOpRequest = ({
  ...
  requestParameters: TestMeOpRequestParameters,
): FetchParameters => {
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/testMe`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }

  const body = JSON.stringify(
    typeSerializer.toJsonObject('SomeObject')(requestParameters.body),
  );
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function testMeOp(
  requestParameters: TestMeOpRequestParameters,
  configuration: OperationConfiguration = {},
): Promise<void> {
  return pipe(
    testMeOpRequest(configuration),
    fetchRequest(configuration),
    testMeOpResponse(configuration),
  )(requestParameters);
}
