...
export const echotestRequest = ({
  ...
  if (
    requestParameters.inputBinaryData === null ||
    requestParameters.inputBinaryData === undefined
  ) {
    throw new APIRequestError(
      `Required parameter requestParameters.inputBinaryData was null or undefined when calling echotest.`,
    );
  }
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/tests/binaryResponse`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }

  const body = requestParameters.inputBinaryData as any;
