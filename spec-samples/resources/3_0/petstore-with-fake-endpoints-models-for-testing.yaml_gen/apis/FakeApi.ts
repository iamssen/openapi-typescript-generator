...
export interface FakeHttpSignatureTestRequestParameters {
  pet: Pet;
  query1?: string;
  header1?: string;
}
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const fakeHttpSignatureTestRequest = ({
  basePath = '',
  querystringStringify = querystring,
  username,
  password,
  ...
  requestParameters: FakeHttpSignatureTestRequestParameters,
): FetchParameters => {
  if (requestParameters.pet === null || requestParameters.pet === undefined) {
    throw new APIRequestError(
      `Required parameter requestParameters.pet was null or undefined when calling fakeHttpSignatureTest.`,
    );
  }
  ...
  if (
    requestParameters.header1 !== undefined &&
    requestParameters.header1 !== null
  ) {
    headerParameters.set('header_1', String(requestParameters.header1));
  }

  let url: string = `${basePath.replace(/\/+$/, '')}/fake/http-signature-test`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const fakeHttpSignatureTestResponse = (
  configuration: ResponseConfiguration = {},
...
export async function fakeHttpSignatureTest(
  ...
  return pipe(
    fakeHttpSignatureTestRequest(configuration),
    fetchRequest(configuration),
    fakeHttpSignatureTestResponse(configuration),
  )(requestParameters);
}
...
export const fakeOuterCompositeSerializeRequest = ({
  ...
  const body = JSON.stringify(
    typeSerializer.toJsonObject('OuterComposite')(
      requestParameters.outerComposite,
    ),
  );
