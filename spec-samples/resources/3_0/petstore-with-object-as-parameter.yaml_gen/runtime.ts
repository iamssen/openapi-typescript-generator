/* eslint-disable */
/**
 * OpenAPI Petstore
 * This is a sample server Petstore server. For this sample, you can use the api key `special-key` to test the authorization filters.
 *
 * The version of the OpenAPI document: 1.0.0
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit the class manually.
 */

import { TypeSerializer } from '@rocket-scripts/openapi';

import { CategoryModel } from './models/Category';
import { ModelApiResponseModel } from './models/ModelApiResponse';
import { OrderModel } from './models/Order';
import { PetModel } from './models/Pet';
import { PetFormModel } from './models/PetForm';
import { StatusEnumModel } from './models/StatusEnum';
import { TagModel } from './models/Tag';
import { UploadFormModel } from './models/UploadForm';
import { UserModel } from './models/User';

export const typeSerializer = new TypeSerializer({
  Category: CategoryModel,
  ModelApiResponse: ModelApiResponseModel,
  Order: OrderModel,
  Pet: PetModel,
  PetForm: PetFormModel,
  StatusEnum: StatusEnumModel,
  Tag: TagModel,
  UploadForm: UploadFormModel,
  User: UserModel,
});
