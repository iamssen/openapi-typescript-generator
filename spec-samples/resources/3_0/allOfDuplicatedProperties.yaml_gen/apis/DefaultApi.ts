...
import type { ModelC } from '../models';
...
) => async (response: Response): Promise<ModelC> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObject('ModelC'));
  } else {
...
export async function testApi(
  ...
  return pipe(
    testApiRequest(configuration),
    fetchRequest(configuration),
    testApiResponse(configuration),
  )();
}
