...
export const op5Request = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/group4/op5`;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function op5(
  configuration: OperationConfiguration = {},
): Promise<void> {
  return pipe(
    op5Request(configuration),
    fetchRequest(configuration),
    op5Response(configuration),
  )();
}
...
export const op6Request = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/group4/op6`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function op6(
  configuration: OperationConfiguration = {},
): Promise<void> {
  return pipe(
    op6Request(configuration),
    fetchRequest(configuration),
    op6Response(configuration),
  )();
}
