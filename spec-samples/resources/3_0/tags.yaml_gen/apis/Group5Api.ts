...
export const op7Request = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/group5/op7`;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function op7(
  configuration: OperationConfiguration = {},
): Promise<void> {
  return pipe(
    op7Request(configuration),
    fetchRequest(configuration),
    op7Response(configuration),
  )();
}
...
export const op8Request = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/group6/op8`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function op8(
  configuration: OperationConfiguration = {},
): Promise<void> {
  return pipe(
    op8Request(configuration),
    fetchRequest(configuration),
    op8Response(configuration),
  )();
}
