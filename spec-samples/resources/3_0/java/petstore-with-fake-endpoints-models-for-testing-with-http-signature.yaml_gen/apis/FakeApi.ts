 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const getArrayOfEnumsRequest = ({
  basePath = '',
  querystringStringify = querystring,
  username,
  password,
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/fake/array-of-enums`;
  if (queryParameters && Object.keys(queryParameters).length > 0) {
    url += '?' + querystringStringify(queryParameters);
  }
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const getArrayOfEnumsResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<Array<OuterEnum>> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObjectList('OuterEnum'));
  } else {
    switch (response.status.toString()) {
      default:
        throw response;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function getArrayOfEnums(
  configuration: OperationConfiguration = {},
): Promise<Array<OuterEnum>> {
  return pipe(
    getArrayOfEnumsRequest(configuration),
    fetchRequest(configuration),
    getArrayOfEnumsResponse(configuration),
  )();
}
