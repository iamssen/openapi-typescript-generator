...
import type { ClubForCreation } from '../models';
...
) => async (response: Response): Promise<ClubForCreation> => {
  if (response.status >= 200 && response.status < 300) {
    return response
      .json()
      .then(typeSerializer.toValueObject('ClubForCreation'));
  } else {
