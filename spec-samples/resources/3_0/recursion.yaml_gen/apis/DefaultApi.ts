 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const fooPostResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<Foo> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObject('Foo'));
  } else {
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function fooPost(
  configuration: OperationConfiguration = {},
): Promise<Foo> {
  return pipe(
    fooPostRequest(configuration),
    fetchRequest(configuration),
    fooPostResponse(configuration),
  )();
}
