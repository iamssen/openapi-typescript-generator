...
import type { ParentType } from '../models';
 ...
 */
export const typeAliasGetRequest = ({
  basePath = '',
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/type-alias`;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const typeAliasGetResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<ParentType> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObject('ParentType'));
  } else {
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export async function typeAliasGet(
  configuration: OperationConfiguration = {},
): Promise<ParentType> {
  return pipe(
    typeAliasGetRequest(configuration),
    fetchRequest(configuration),
    typeAliasGetResponse(configuration),
  )();
}
