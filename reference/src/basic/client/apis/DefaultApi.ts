...
import type { SomeData, SomeException } from '../models';
...
export const someGetRequest = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/some`;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const someGetResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<SomeData> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObject('SomeData'));
  } else {
    switch (response.status.toString()) {
      case '400': // fail
        throw new APIExceptionError({
          exception: typeSerializer.toValueObject('SomeException')(
            await response.json(),
          ),
          message: `fail`,
          response,
...
export async function someGet(
  ...
  return pipe(
    someGetRequest(configuration),
    fetchRequest(configuration),
    someGetResponse(configuration),
  )();
}
