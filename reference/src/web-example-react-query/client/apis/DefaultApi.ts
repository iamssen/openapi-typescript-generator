...
export const postsGetRequest = ({
  ...
  let url: string = `${basePath.replace(/\/+$/, '')}/posts`;
 ...
 * @throws {APISerializeError} throw when fail serialize data (include request and response)
 */
export const postsGetResponse = (
  configuration: ResponseConfiguration = {},
) => async (response: Response): Promise<Array<Post>> => {
  if (response.status >= 200 && response.status < 300) {
    return response.json().then(typeSerializer.toValueObjectList('Post'));
  } else {
...
export async function postsGet(
  ...
  return pipe(
    postsGetRequest(configuration),
    fetchRequest(configuration),
    postsGetResponse(configuration),
  )();
}
