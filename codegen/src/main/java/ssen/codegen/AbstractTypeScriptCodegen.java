...
public abstract class AbstractTypeScriptCodegen extends DefaultCodegen {
    ...
    // NOTE: SimpleDateFormat is not thread-safe and may not be static unless it is
    // thread-local
    @SuppressWarnings("squid:S5164")
    protected static final ThreadLocal<SimpleDateFormat> SNAPSHOT_SUFFIX_FORMAT = ThreadLocal
            .withInitial(() -> new SimpleDateFormat("yyyyMMddHHmm", Locale.ROOT));
    ...
    public AbstractTypeScriptCodegen() {
        ...
        modifyFeatureSet(features -> features.includeDocumentationFeatures(DocumentationFeature.Readme)
                ...
                .excludeGlobalFeatures(GlobalFeature.XMLStructureDefinitions, GlobalFeature.Callbacks,
                        GlobalFeature.LinkObjects, GlobalFeature.ParameterStyling)
        ...
        // to support multiple inheritance e.g. export interface ModelC extends ModelA,
        // ModelB
        // supportsMultipleInheritance = true;

        // NOTE: TypeScript uses camel cased reserved words, while models are title
        // cased. We don't want lowercase comparisons.
        ...
        languageSpecificPrimitives = new HashSet<>(
                Arrays.asList("string", "String", "boolean", "Boolean", "Double", "Integer", "Long", "Float", "Object",
                        "Array", "ReadonlyArray", "Date", "number", "any", "File", "Error", "Map", "object", "Set"));

        languageGenericTypes = new HashSet<>(Collections.singletonList("Array"));
        ...
        cliOptions.add(new CliOption(CodegenConstants.SUPPORTS_ES6, CodegenConstants.SUPPORTS_ES6_DESC)
                .defaultValue(String.valueOf(this.getSupportsES6())));
        this.cliOptions.add(new CliOption(NPM_NAME, "The name under which you want to publish generated npm package."
                + " Required to generate a full package"));
        this.cliOptions.add(new CliOption(NPM_VERSION,
                "The version of your npm package. If not provided, using the version from the OpenAPI specification file.")
                        .defaultValue(this.getNpmVersion()));
        this.cliOptions.add(CliOption.newBoolean(SNAPSHOT,
                "When setting this property to true, the version will be suffixed with -SNAPSHOT."
                        + AbstractTypeScriptCodegen.SNAPSHOT_SUFFIX_FORMAT.get().toPattern(),
                false));
        this.cliOptions.add(new CliOption(NULL_SAFE_ADDITIONAL_PROPS, NULL_SAFE_ADDITIONAL_PROPS_DESC)
    ...
    protected void supportModelPropertyNaming(MODEL_PROPERTY_NAMING_TYPE defaultModelPropertyNaming) {
        ...
        cliOptions
                .add(new CliOption(CodegenConstants.MODEL_PROPERTY_NAMING, CodegenConstants.MODEL_PROPERTY_NAMING_DESC)
                        .defaultValue(modelPropertyNaming.name()));
    }
    ...
    public void processOpts() {
        ...
        if (StringUtils.isEmpty(System.getenv("TS_POST_PROCESS_FILE"))) {
            LOGGER.info(
                    "Hint: Environment variable 'TS_POST_PROCESS_FILE' (optional) not defined. E.g. to format the source code, please try 'export TS_POST_PROCESS_FILE=\"/usr/local/bin/prettier --write\"' (Linux/Mac)");
            LOGGER.info(
                    "Note: To enable file post-processing, 'enablePostProcessFile' must be set to `true` (--enable-post-process-file for CLI).");
        } else if (!this.isEnablePostProcessFile()) {
            LOGGER.info(
                    "Warning: Environment variable 'TS_POST_PROCESS_FILE' is set but file post-processing is not enabled. To enable file post-processing, 'enablePostProcessFile' must be set to `true` (--enable-post-process-file for CLI).");
        }
        ...
        if (additionalProperties.containsKey(NULL_SAFE_ADDITIONAL_PROPS)) {
            setNullSafeAdditionalProps(
                    Boolean.valueOf(additionalProperties.get(NULL_SAFE_ADDITIONAL_PROPS).toString()));
        }
    ...
    public void preprocessOpenAPI(OpenAPI openAPI) {
        ...
        if (additionalProperties.containsKey(NPM_NAME)) {
            ...
            // If no npmVersion is provided in additional properties, version from API
    ...
    protected String toTypescriptTypeName(final String name, String safePrefix) {
        ...
        // this is unlikely to happen, because we have just camelized the name, while
        // reserved words are usually all lowcase
        ...
        if (sanName.matches("^\\d.*")) {
            ...
            LOGGER.warn(
                    sanName + " (model name starts with number) cannot be used as model name. Renamed to " + modelName);
            return modelName;
        }
        ...
        if (languageSpecificPrimitives.contains(sanName)) {
            String modelName = safePrefix + sanName;
            LOGGER.warn(
                    sanName + " (model name matches existing language type) cannot be used as a model name. Renamed to "
                            + modelName);
            return modelName;
        }
    ...
    protected String getParameterDataType(Parameter parameter, Schema p) {
        ...
        /*
         * TODO revise the logic below else if (ModelUtils.isDateSchema(p)) { // Handle
         * date enums DateSchema sp = (DateSchema) p; if (sp.getEnum() != null) { return
         * enumValuesToEnumTypeUnion(sp.getEnum(), "string"); } } else if
         * (ModelUtils.isDateTimeSchema(p)) { // Handle datetime enums DateTimeSchema sp
         * = (DateTimeSchema) p; if (sp.getEnum() != null) { return
         * enumValuesToEnumTypeUnion(sp.getEnum(), "string"); } }
         */
    ...
    protected boolean isReservedWord(String word) {
        // NOTE: This differs from super's implementation in that TypeScript does _not_
    ...
    public void setModelPropertyNaming(String naming) {
        try {
            ...
            String values = Stream.of(MODEL_PROPERTY_NAMING_TYPE.values()).map(value -> "'" + value.name() + "'")
                    .collect(Collectors.joining(", "));

            String msg = String.format(Locale.ROOT, "Invalid model property naming '%s'. Must be one of %s.", naming,
                    values);
    ...
    protected void setEnumPropertyNaming(String naming) {
        try {
            ...
            String values = Stream.of(ENUM_PROPERTY_NAMING_TYPE.values()).map(value -> "'" + value.name() + "'")
                    .collect(Collectors.joining(", "));

            String msg = String.format(Locale.ROOT, "Invalid enum property naming '%s'. Must be one of %s.", naming,
                    values);
            throw new IllegalArgumentException(msg);
        }
    }
            ...
            for (Map<String, Object> mo : models) {
                ...
                if (cm.discriminator != null && cm.children != null) {
                    for (CodegenModel child : cm.children) {
                        this.setDiscriminatorValue(child, cm.discriminator.getPropertyName(),
                                this.getDiscriminatorValue(child));
                    }
                }
            }
        }
    ...
    private void setDiscriminatorValue(CodegenModel model, String baseName, String value) {
        ...
        if (model.children != null) {
            ...
            for (CodegenModel child : model.children) {
                this.setDiscriminatorValue(child, baseName,
                        newDiscriminator ? value : this.getDiscriminatorValue(child));
            }
        }
    }
